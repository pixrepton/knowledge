...
| `INTELLIGENCE-SPINE-P1-1P`                | gmail-agent / knowledge     | **CLOSED**                 | P1.1P DURABLE REVISION STATE 2026-08-22: Delivery COMPLETE, Proof PASS_LOCAL_BOUNDED, Full Gate A PASS (2769/15/24/0). Decision revision lineage persisted in existing MailboxMemoryStore (2 tables in the same store, no new DB); ledger = projection/cache with rebuild() + fail-closed (0 or >1 CURRENT -> rebuild_one_current_violation; ordering by revision integer, never timestamp); accept transition atomic (old SUPERSEDED / new CURRENT / request ACCEPTED). Restart recovery proven: current=r2, r1 SUPERSEDED, duplicate request -> DUPLICATE_REVISION_REQUEST (no r3), stale -> STALE_REVISION_REQUEST, old approval/ToolPlan -> DENY STALE_DECISION_REVISION. Commit: gmail-agent f57028ea. Artefakt: `.artifacts/intelligence-spine-p1-1p-20260822T110000/restart-trajectory.json`. P1.2-P1.5 NOT_STARTED. |
