...
| `INTELLIGENCE-SPINE-P0-CAD`               | gmail-agent / knowledge     | **IN_PROGRESS**            | P0 `AI-OS INTELLIGENCE SPINE — CONTRACT + FIRST ENFORCED SLICE`: `BusinessDecisionProposal` -> `CanonicalizationFailure` (przed CAD) / `CanonicalActionDecision` (FROZEN, decision_id+semantic_hash) -> ActionPlan/NBA re-role -> policy authority only -> capability filtering (effective_tools nie oferuje ROC dla customer/mail) -> reference monitor (canonical_semantic_drift) -> deterministyczne property tests. Kontrakt: `docs/AI_OS_INTELLIGENCE_SPINE_CONTRACT.md`. Bez Full Fresh38. |
