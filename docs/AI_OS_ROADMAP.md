...
P1.5 + P1.5B SUBJECT-AWARE PROPOSITION IDENTITY (2026-08-23, closeout):
kanoniczny owner faktów = MailboxMemoryStore (InMemory+Postgres) + wspólny
semantic owner `mailbox_memory.facts` / `split_conflicting_facts`.
Centralny invariant: evidence identity != subject identity != proposition
identity. Dla subject-aware facts proposition identity =
`(subject_kind, subject_id, fact_key)`; `entity_scope` pozostaje storage /
projection dimension, a `message_id` / `document_id` / `source_ref`
pozostają evidence-only. Bounded SubjectRef:
`kind={CASE,CUSTOMER,PROPERTY,DEVICE,SERVICE_EVENT}`,
`resolution={EXPLICIT,SINGLE_SUBJECT_DEFAULT,AMBIGUOUS}`. Same-subject
cross-source same-value -> one proposition + preserved evidence refs;
different subjects -> no false conflict; unknown subject -> fail-closed
(`AMBIGUOUS`) i brak zgadywania; legal supersession jest subject-local.
Producer wiring objęło mail/customer, structured attachments, document
intelligence, drive promotion oraz case/internal writes przez
`attach_subject_metadata`. Pack/snapshot/P1.3 korzystają z tej samej
semantyki. Full Gate A PASS (0 failed) na finalnym HEAD; SPINE_CORE PASS;
POSTGRES_SUBJECT_IDENTITY_PROOF PASS (real Postgres restart round-trip,
rozdzielony od Gate A). Artefakty:
`.artifacts/intelligence-spine-p1-5-20260823T100000/`
(`p1-5b-subject-taxonomy-audit.json`,
`bounded-subject-identity-trajectory.json`). Status:
P1.5 COMPLETE / PASS_LOCAL_BOUNDED; P1.5B COMPLETE / PASS_LOCAL_BOUNDED;
P1.3 FROZEN; P1.4 COMPLETE.
...
| --------- | --------------- | ------------------------------------------------------------ | ----------------------------------------------------------------------------------- |
| **P1**    | Infrastructure  | `AI_OS_INFRASTRUCTURE_CLEANUP`                               | **COMPLETE_LOCAL** — `OPERATOR-COMMAND-RECONCILE-BYPASS-01` closed; `REQUIRED_OPEN = 0`, `UNKNOWN_NEEDS_PROOF = 0`; Gate A gmail-agent 0 failed |
| **P2**    | Residuals       | no new residual wave                                         | closed residuals remain closed unless new regression evidence appears              |
| **P3**    | Product quality | Fresh38 requalified (measurement) — **11-CAPABILITY closed; SVC-05 CLOSED** | Measurement green (38/38, 2026-08-16); baseline 27/38 CLEAN_PASS (v5, próg 34) below threshold; 11 residuals fixed/classified with focused/bounded proofs; SVC-05 final provider-live proof PASS; no full Fresh38 |
...
| Data       | Zmiana                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-20 | **AI-OS FULL WORKSPACE TRUTH SYNC — CLOSED_PASS.** Current-state writeback after SVC-05 closeout: 11-CAPABILITY product residuals open = 0; Node B local health OK, mailbox memory healthy; RAG backend not current healthy and full core preflight is not a current PASS until RAG health is re-proven; GitNexus active repo indexes refreshed to current HEAD as CLI/index; CBM MCP query-plane callable and active repo heads current; CodeScene configured but unauthenticated. Root `LOCAL_ONLY/no remote` and old gmail-only CBM claims superseded. Active branches pushed; local ↔ GitHub parity confirmed (`0 0`) for workspace, knowledge and all active nested repos. No full Fresh38. |
| 2026-08-20 | **SVC-05 CLOSED; 11-CAPABILITY closed.** Product fix PASS (`0a407cb3`), downstream ActionPlan/planner preservation PASS (`78603fb` + final `70c3d94`), final provider-live proof PASS: SVC-05/SVC-02/MI-03 = 3/3 QUALIFIED; SVC-05 chain `collect_data -> DRAFT_ACCEPTED -> prepare_reply -> ask_for_missing_data/mail -> generate_draft_reply -> HITL`, `operator_clarification_requested=false`. No full Fresh38. |
| 2026-08-16 | **AI-OS WORKSPACE CONSOLIDATION & CANONICAL SYNC — PASS.** Control-plane: AGENTS/CLAUDE + hook guards + `post-run-knowledge-writeback` skill (registered w/ AGENT_MAP_SCENARIOS, **nie** core_skills) + kalk-top AGENTS.md Typ A fix (`agent_harness_audit` 147/0 PASS). Knowledge: Fresh38 writeback + harness terminology (single owner `MEASUREMENT_INTEGRITY_V3.md`) + eval artifacts + LF policy. Per-repo `.gitattributes` LF pin (phantom CRLF cleared; 3 byte-identical phantoms documented). Published 8 repo (`LOCAL==REMOTE` verified; root `INTENTIONAL_LOCAL_ONLY`; kalk-top push → własny task). GitNexus root/gmail-agent/fast-kalk reindexed; CBM gmail-agent refreshed; CodeScene `UNAVAILABLE_AUTH`. Baseline bez zmian: 27/11 (v5). Task `AIOS-WORKSPACE-CONSOLIDATION-20260813` closed. | 
| 2026-08-16 | **Fresh38 measurement REQUALIFIED — PASS (measurement).** L0 kontrakt + tożsamość harnessa (`PROVEN_FULL_RUN_HARNESS=3b3041e5`, `COMMITTED_HARNESS=035534a7…`, `IMPLEMENTATION_HARDENING_AFTER_FULL_PROOF=YES`) — canonical owner `docs/MEASUREMENT_INTEGRITY_V3.md`. Deterministic gates 6/6; four-case PASS; full 38/38 QUALIFIED (`fresh38_full_current_20260816T124100`); L3 judge 30/30 SCORED + rescore v5: **CLEAN_PASS=27 / CAPABILITY=11 → NOT QUALIFIED — CAPABILITY** (próg 34; produkt osobno) — **freeze baseline**; **nie nazywać „23→27 improvement”** (run nie SUT-identyczny). `PRODUCT_SEMANTICS_CHANGE=0`; SUT `gmail-agent@37d4b37`. Administracyjne zamknięcie: regression 231 + gates + commit `4210ed5` + checkpoint CLOSED. Next: 11 CAPABILITY cases. | 
| 2026-08-09 | **AI-OS infrastructure cleanup — COMPLETE_LOCAL.** Follow-up after `OPERATOR-COMMAND-RECONCILE-BYPASS-01` closeout: remote SHAs synced, Gate A gmail-agent green with 0 failed, `REQUIRED_OPEN = 0`, `UNKNOWN_NEEDS_PROOF = 0`. Formal posture: `AI_OS_INFRASTRUCTURE_CLEANUP=COMPLETE_LOCAL`; `CAPABILITY_PROGRAM_READINESS=GO`. Next real program is Fresh38 28-CAPABILITY, not another residual wave. |
