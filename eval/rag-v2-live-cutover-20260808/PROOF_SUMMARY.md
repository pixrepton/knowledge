...
- Operator **already authorized** staged activation (`ZEZWALAM NA ACTIVATION`).
- Execution status: **`STAGED_ACTIVATION_AUTHORIZED — BLOCKED_BY_TECHNICAL_GATE`**
  (`RAG-TEMPORAL-COMPLETE-01` + `RAG-IMAGE-BAKE-01`). Not `OPERATOR_DECISION_REQUIRED`.
...
Technical recommendation remains **B — staged cutover** (`technical_manual`, `price_list`)
via opt-in compose file only.

Operator chat authorization **already exists** — do **not** re-ask. Staged execution waits on
stable Temporal COMPLETE + reproducible image bake (or explicit acceptance of PARTIAL residual).

Suggested activation (when technical gates PASS):
...
| RAG-09 | COMPLETE_BOUNDED | PROVEN_RUNTIME (opt-in + rollback)                   | STAGED_ACTIVATION_AUTHORIZED — BLOCKED_BY_TECHNICAL_GATE |
